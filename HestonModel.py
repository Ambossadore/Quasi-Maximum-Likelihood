...
class KalmanFilter:
    def __init__(self, dim, a, A, BB, BB_lim=None, first_observed=0):
        self.dim = dim
        self.a = a
        self.A = A
        self.BB = BB
    ...
    def build_covariance(self, t_max=1000):
        Sig_tp1_t_list = [np.zeros([self.dim, self.dim]), self.BB(t=1)]
        ...
        for t in range(1, t_max):
            ...
            Sig_tp1_t = self.A() @ Sig_tt @ self.A().T + self.BB(t=t + 1)
    ...
    def S_star(self, wrt):
        BB_lim_partial = self.BB_lim(wrt=wrt, order=1)
    ...
    def R_star(self, wrt):
        BB_lim_partial2 = self.BB_lim(wrt=wrt, order=2)
    ...
    def deriv2_filter_hom(self, x, observations, wrt, t_max=None, verbose=0, close_pb=True, deriv_filters=None):
        if t_max is None:
            t_max = observations.shape[0]
        Sig_inv = np.linalg.inv(self.Sig_tp1_t_lim[self.first_observed:, self.first_observed:])
        s_star = self.S_star(wrt=wrt)
        ...
        if deriv_filters is not None:
            ...
            V_tp1_t, V_tt = self.deriv_filter_hom(x, observations, wrt=wrt, t_max=t_max, verbose=verbose, close_pb=close_pb)
        ...
        W_tp1_t_list = [np.zeros((partial_a.shape[0], self.dim)), partial_a + partial_A @ x]
        W_tt_list = [np.zeros((partial_a.shape[0], self.dim))]
...
class PolynomialModel:
    ...
    def log_lik(self, param, t, verbose=0):
        x = self.x
        kfilter = KalmanFilter(dim=self.dim, a=partial(self.a, param=param), A=partial(self.A, param=param), BB=partial(self.C, param=param), first_observed=self.first_observed)
        kfilter.build_covariance(t_max=t)
        kfilter.build_kalman_filter(x=x, observations=self.observations, t_max=t, verbose=verbose)
    ...
    def score(self, param, t, verbose=0, kfilter=None):
        x = self.x
        kfilter = KalmanFilter(dim=self.dim, a=partial(self.a, param=param), A=partial(self.A, param=param), BB=partial(self.C, param=param), first_observed=self.first_observed)
        kfilter.build_covariance(t_max=t)
        kfilter.build_kalman_filter(x=x, observations=self.observations, t_max=t, verbose=verbose)
    ...
    def fit_qml(self, fit_parameter, t=None, verbose=1):
        global it
        it = 1
        ...
        with warnings.catch_warnings():
            ...
            if verbose == 1:
                res = minimize(neg_loglik, self.true_param[fit_parameter], bounds=bounds, callback=callback, method='L-BFGS-B')
            else:
                res = minimize(neg_loglik, self.true_param[fit_parameter], bounds=bounds, method='L-BFGS-B')
        end = time.time()
        if verbose == 1:
    ...
    def fit_qml_sequence(self, fit_parameter, t_max=None, every=50, verbose=1):
        start = time.time()
        ...
        if os.path.exists(filename):
            ...
            for t in t_range:
                ...
                qml_list.append(self.fit_qml(fit_parameter=fit_parameter, t=t, verbose=verbose))
    ...
    def compute_U(self, param, wrt=np.array([0, 1, 2, 3]), t_max=None, verbose=0, filter_unobserved=False, kfilter=None, deriv_filters=None, close_pb=True, save_raw=False):
        ...
        filepath = self.model_name + '/Covariance/U_raw{}_firstobserved{}_seed{}_{}obs.pkl'.format(np.atleast_1d(wrt).tolist(), self.first_observed, self.seed, t_max)
        ...
        if kfilter is None:
            ...
            kfilter = KalmanFilter(dim=self.dim, a=a, A=A, BB=BB, BB_lim=BB_lim, first_observed=self.first_observed)
        ...
        ### Checkup
        # beta_f = -0.5 * np.trace(Sig_inv @ kfilter.S_star(wrt=wrt)[self.first_observed:, self.first_observed:])
        # goal = self.cond_score_hom(param, t=24, wrt=2)
        # Gamma @ np.kron(hatX[24], hatX[24]) + beta_f
...
class HestonModel(PolynomialModel):
    ...
    def generate_observations(self, t_max, seed=None, verbose=0):
        ...
        np.savetxt('HestonModel/Observations/observations_{}_seed{}_{}obs.txt'.format(self.savestring, seed, t_max), observations)
    ...
    def from_observations(cls, first_observed, kappa, theta_vol, sig, rho, v0_vol, obs, seed=0):
        filename = 'HestonModel/Observations/observations_{}_{}_{}_{}_seed{}_{}obs.txt'.format(kappa, theta_vol, sig, rho, seed, obs)
