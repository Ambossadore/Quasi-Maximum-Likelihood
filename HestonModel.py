...
class PolynomialModel:
    ...
    def compute_U(self, kind='explicit', wrt=None, t_max=None, verbose=0, filter_unobserved=False, kfilter=None, deriv_filters=None, close_pb=True, save_raw=False):
        if wrt is None:
            if self.wrt is None:
    ...
    def compute_V(self, kind='explicit', wrt=None, t_max=None, verbose=0, filter_unobserved=False, from_raw=None, every=None):
        if wrt is None:
            if self.wrt is None:
                raise Exception('Method setup_filter needs to be called first or argument wrt has to be specified.')
            wrt = self.wrt
...
class HestonModel(PolynomialModel):
    ...
    def generate_more_observations(self, t_max, dt, seed=None, verbose=0):
        if seed is not None:
            np.random.seed(seed)
        if np.isnan(self.true_param).any():
            raise Exception('Full underlying parameter has to be given or has to be estimated first.')
        if self.observations is None:
            raise Exception('No observations available. Please call method "generate_observations" first or provide external observations')
        kappa, theta, sigma, rho = self.true_param
